...
func main() {
	...
	traceRI := flag.String("trace-ri", "", "Log every Reserved Instruction exception (PC, raw word, disassembly) to this file")
	...
	if *traceRI != "" {
		f, err := os.Create(*traceRI)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error opening RI trace file: %v\n", err)
			os.Exit(1)
		}
		defer f.Close()
		m.CPU.EnableRITrace(f)
		fmt.Printf("Logging Reserved Instruction exceptions to: %s\n", *traceRI)
	}
	...
	if *traceRI != "" {
		counters := m.CPU.RICounters()
		fmt.Println("\n--- Reserved Instruction Summary ---")
		if len(counters) == 0 {
			fmt.Println("  <none>")
		} else {
			var total uint64
			...
			fmt.Printf("  %d unique sites, %d total exceptions\n\n", len(counters), total)
			fmt.Printf("  %-10s %-12s %-12s %s\n", "Count", "Last PC", "Raw", "Key")
			for _, ct := range counters {
				fmt.Printf("  %-10d 0x%08x   0x%08x   %s\n", ct.Count, ct.LastPC, ct.Raw, ct.Key)
			}
		}
	}
...
func reportPeripherals(blocks map[string]*device.RegisterBlock) {
	...
	for _, name := range []string{"CPM", "INTC", "TCU", "OST", "GPIO", "I2C0", "DDRC", "DDRP", "SFC", "GMAC", "DWC2", "EFUSE", "MSC", "PERIPH"} {
		block := blocks[name]

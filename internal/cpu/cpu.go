...
type CPU struct {
	...
	// idleSpins counts consecutive executions of the kernel idle-loop
	// branch-back. The Ingenic T23 kernel's idle loop busy-waits
	// instead of calling WAIT, so the WAIT fast-forward never fires.
...
func (c *CPU) Step() {
	...
	// Kernel idle-loop fast-forward. The Ingenic T23 kernel's idle
	// loop at 0x80068130 busy-waits because a flag at 0x80426784
	// (set to 1 at boot) forces the work-processing path instead of
	// the WAIT path. Detect the loop-back branch at 0x800682EC and,
	// after a few iterations, skip to the next timer interrupt.
	if pc == 0x800682EC {
		c.idleSpins++
		if c.idleSpins >= 10 {
			c.idleSpins = 0
